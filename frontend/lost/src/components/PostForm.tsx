...
const PostForm: React.FC<PostFormProps> = ({ mode, initialPost, postId, onClose, onSuccess }) => {
  ...
  const [post, setPost] = useState<Post>(initialPost || { title: '',  description: '', location: '', phone_number: '', owner_name:''});
    ...
    <div className="max-w-lg mx-auto p-4">
      ...
      {mode !== 'delete' ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="owner_name" className="block font-medium">Owner Name</label>
            <input
              id="owner_name"
              value={post.owner_name}
              onChange={(e) => setPost({ ...post, owner_name: e.target.value })}
              className="w-full px-3 py-1 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
            />
          </div>
